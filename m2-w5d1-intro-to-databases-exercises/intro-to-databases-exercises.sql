...
SELECT name, continent, headofstate FROM country WHERE governmentform IN ('Monarchy', 'Monarchy (Emirate)', 'Monarchy (Sultanate)', 'Constitutional Monarchy', 'Constitutional Monarchy (Emirate)', 'Constitutional Monarchy, Federation', 'Parlementary Monarchy')
...
SELECT name, gnp / population * 1000000 AS per_capita_gnp_in_US_dollars FROM country WHERE continent = 'Europe'
-- 22. The number of years since independence for all countries that have a year of independence (192 rows)
SELECT name, 2017 - indepyear AS number_of_years_since_independence FROM country WHERE indepyear IS NOT NULL